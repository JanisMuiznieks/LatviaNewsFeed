...
class NewsFeedViewController: UIViewController {
    ...
    func handleGetData(){
        let jsonUrl =  "https://newsapi.org/v2/top-headlines?country=lv&apiKey=426785172f6a46d18a6283d67806fde0"
        ...
        let task = session.dataTask(with: urlRequest) { (data, request, error) in
            ...
            guard let data = data else { print("Data error!!!!")
                return
            }
            do {
    ...
    func populateData(_ dict: [String: Any]){
        guard let responseDict = dict["articles"] as? [Gloss.JSON]
        else {
            return
        }
...
extension NewsFeedViewController : UITableViewDataSource, UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        if let vc = UIStoryboard(name: "Main", bundle: Bundle.main).instantiateViewController(identifier: "DetailViewController") as? DetailViewController {
            ...
            vc.imageURLString = items[indexPath.row].image
